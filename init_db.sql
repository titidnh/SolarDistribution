-- ═══════════════════════════════════════════════════════════════════════════════
-- SolarDistribution — Schéma MariaDB complet
-- Inclut : ML-7 (contexte adaptatif), ML-8 (prévisions HA installation-spécifiques)
-- ═══════════════════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS distribution_sessions (
    ...
    hours_remaining_in_slot     DECIMAL(5,2)    NULL    COMMENT 'Heures restantes dans le créneau HC',
    hours_until_solar           DECIMAL(5,2)    NULL    COMMENT 'Heures avant ensoleillement (null=non prévu)',
    had_emergency_grid_charge   TINYINT(1)      NOT NULL DEFAULT 0 COMMENT 'Au moins 1 batterie en urgence réseau',
    effective_grid_charge_w     DECIMAL(8,2)    NULL    COMMENT 'Puissance réseau adaptative moyenne hors urgence (W)',
...
CREATE TABLE IF NOT EXISTS battery_snapshots (
    ...
    is_emergency_grid_charge    TINYINT(1)      NOT NULL DEFAULT 0 COMMENT 'Charge urgence SOC critique',
    grid_charge_allowed_w       DECIMAL(8,2)    NOT NULL DEFAULT 0 COMMENT 'Puissance réseau adaptative autorisée (W)',
